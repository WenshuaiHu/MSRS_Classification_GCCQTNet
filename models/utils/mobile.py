...
from models.utils.utils import MyDyRelu
...
class Mobile(nn.Module):
    def __init__(self, ks, inp, hid, out, se, stride, dim, reduction=4, k=2):
        super(Mobile, self).__init__()### DyRelu k=2
        self.hid = hid
        self.k = k
...
class MobileDown(nn.Module):
    ...
    def forward(self, x, z):
        # print(11111111111111111, x.size(), z.size())
        # torch.Size([100, 128, 11, 11]) torch.Size([100, 6, 128])
        theta = self.get_relu_coefs(z)
        # b 2*k*c -> b c 2*k                                     2*k            2*k
        relu_coefs = theta.view(-1, self.hid, 2 * self.k) * self.lambdas + self.init_v
        ...
        #print(111111111111111111111, out.size())
